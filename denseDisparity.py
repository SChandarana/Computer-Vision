...
def disparity_map(l_img_g, r_img_g, WLS=False):
    ...
    if WLS: #if we want to use WLS use then this will run
        #following the WLS tutorial
    ...
    dispNoiseFilter = 5 #remove noise
    cv2.filterSpeckles(disparity, 0, 4000, max_disparity - dispNoiseFilter)

    _, disparity = cv2.threshold(disparity,0, max_disparity * 16, cv2.THRESH_TOZERO)
    disparity_scaled = (disparity / 16.).astype(np.uint8)

    disparity_scaled = disparity_scaled[0:390] #crop bonnet
...
def distance_from_disparity(disparity, box): #disparity = disparity map, box = object detected via yolo
    ...
    feature_sub_array = disparity[max(top,0):min(top+height+1,dispH),max(136,left):min(left+width+1,dispW)] #making sure the features boxes are within the image 

    #average_feature_disparity = np.nanmean(feature_sub_array)
    #average_feature_disparity = np.nanmedian(feature_sub_array)
